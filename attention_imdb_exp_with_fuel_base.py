...
def imdb_exp(datasets, 
    ...
    if unpacked:
        ...
        input_shape = (timesteps,  max_words)
...
def set_time_step_shape(minibatch):
    rval = []
    # print minibatch[0].shape
    for source_minibatch in minibatch:
...
def prepare_minibatch(minibatch):
    minibatch = set_time_step_shape(minibatch)
    # anything else?
...
def load_datasets(aggregation_files, min_word_count, batch_size,
    ...
    for partition, source in zip(partition_names, partition_sources):
        ...
        data_stream = dataset.mini_batch_stream(
            ...
            maximum_hierarchical_batch_size=42000)

        # print data_stream.sources
        # import pdb; pdb.set_trace()

        data_stream = Mapping(data_stream, prepare_minibatch)
        ...
        # adhot fix: for IMDB, we use some simple post processing to transform the tree tensor
